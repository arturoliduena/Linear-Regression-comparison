...
def lasso_linear_regression_CV(X, y):
  ...
  model = make_pipeline(StandardScaler(), LassoCV(tol=1e-9, max_iter = 5000, cv=20, n_jobs=-1)).fit(X, y)
...
def lasso_linear_regression_LarsCV(X, y):
  start_time = time.time()
  model = make_pipeline(StandardScaler(), LassoLarsCV(max_iter = 5000, cv=20, n_jobs=-1)).fit(X, y)
  fit_time = time.time() - start_time
